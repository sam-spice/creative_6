...
exports.postEntry = function(req, res) {
  User.findOne({ _id: req.session.user })
    .exec(function(err, user) {
      ...
      var journal = new Journal({ user: req.session.user, journal_content: req.body.journal, title: req.body.title });
...
exports.getEntries = function(req, res){
  Journal.find({user: req.session.user}).exec(function(err, journals) {
      if (!journals) {
        res.json([]);
      }
      else {
